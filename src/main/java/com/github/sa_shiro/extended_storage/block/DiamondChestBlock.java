...
public class DiamondChestBlock extends Block {
    ...
    public DiamondChestBlock(Material material, SoundType sound, float hardness, float resistance, int harvestLevel) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return DeferredRegistryEvent.DIAMOND_CHEST_TILE_ENTITY.get().create();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (tileEntity instanceof DiamondChestTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (DiamondChestTileEntity) tileEntity, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            TileEntity tileEntity = worldIn.getTileEntity(pos);
            if (tileEntity instanceof DiamondChestTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((DiamondChestTileEntity) tileEntity).getItems());
            }
        }
    }
