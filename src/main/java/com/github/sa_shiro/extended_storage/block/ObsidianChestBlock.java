...
public class ObsidianChestBlock extends Block {
    ...
    public ObsidianChestBlock(Material material, SoundType sound, float hardness, float resistance, int harvestLevel) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return DeferredRegistryEvent.OBSIDIAN_CHEST_TILE_ENTITY.get().create();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (tileEntity instanceof ObsidianChestTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (ObsidianChestTileEntity) tileEntity, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            TileEntity tileEntity = worldIn.getTileEntity(pos);
            if (tileEntity instanceof ObsidianChestTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((ObsidianChestTileEntity) tileEntity).getItems());
            }
        }
    }
