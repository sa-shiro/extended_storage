...
public class DragonChestBlock extends Block {
    ...
    public DragonChestBlock(Material material, SoundType sound, float hardness, float resistance, int harvestLevel) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return DeferredRegistryEvent.DRAGON_CHEST_TILE_ENTITY.get().create();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (tileEntity instanceof DragonChestTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (DragonChestTileEntity) tileEntity, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            TileEntity tileEntity = worldIn.getTileEntity(pos);
            if (tileEntity instanceof DragonChestTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((DragonChestTileEntity) tileEntity).getItems());
            }
        }
    }
