...
public class EmeraldChestBlock extends Block {
    ...
    public EmeraldChestBlock(Material material, SoundType sound, float hardness, float resistance, int harvestLevel) {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return DeferredRegistryEvent.EMERALD_CHEST_TILE_ENTITY.get().create();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (tileEntity instanceof EmeraldChestTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (EmeraldChestTileEntity) tileEntity, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            TileEntity tileEntity = worldIn.getTileEntity(pos);
            if (tileEntity instanceof EmeraldChestTileEntity) {
                InventoryHelper.dropItems(worldIn, pos, ((EmeraldChestTileEntity) tileEntity).getItems());
            }
        }
    }
