...
import com.github.sa_shiro.extended_storage.container.ObsidianChestContainer;
...
public class ObsidianChestTileEntity extends LockableLootTileEntity {

    private static final int rows = ObsidianChestContainer.numRows;
    private static final int columns = ObsidianChestContainer.numColumns;
    protected int playersUsing;
    private String translationKey = "container.obsidian_chest";
    private NonNullList<ItemStack> chestContent = NonNullList.withSize(rows * columns, ItemStack.EMPTY);
    private IItemHandlerModifiable items = createHandler();
    private LazyOptional<IItemHandlerModifiable> itemHandler = LazyOptional.of(() -> items);

    public ObsidianChestTileEntity(TileEntityType<?> typeIn) {
        super(typeIn);
    }

    public ObsidianChestTileEntity() {
        this(DeferredRegistryEvent.OBSIDIAN_CHEST_TILE_ENTITY.get());
    }

    public static void swapContents(ObsidianChestTileEntity tileEntityIn, ObsidianChestTileEntity tileEntityOut) {
    ...
    public int getPlayersUsing(IBlockReader reader, BlockPos pos) {
        ...
        if (blockState.hasTileEntity()) {
            ...
            if (tileEntity instanceof ObsidianChestTileEntity) {
                return ((ObsidianChestTileEntity) tileEntity).playersUsing;
            }
        }
        return 0;
    }
    ...
    protected Container createMenu(int id, PlayerInventory player) {
        return new ObsidianChestContainer(id, player, this);
    }
