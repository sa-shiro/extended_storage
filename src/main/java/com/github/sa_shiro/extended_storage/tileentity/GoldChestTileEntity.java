...
import com.github.sa_shiro.extended_storage.container.GoldChestContainer;
...
public class GoldChestTileEntity extends LockableLootTileEntity {

    private static final int rows = GoldChestContainer.numRows;
    private static final int columns = GoldChestContainer.numColumns;
    protected int playersUsing;
    private String translationKey = "container.gold_chest";
    private NonNullList<ItemStack> chestContent = NonNullList.withSize(rows * columns, ItemStack.EMPTY);
    private IItemHandlerModifiable items = createHandler();
    private LazyOptional<IItemHandlerModifiable> itemHandler = LazyOptional.of(() -> items);

    public GoldChestTileEntity(TileEntityType<?> typeIn) {
        super(typeIn);
    }

    public GoldChestTileEntity() {
        this(DeferredRegistryEvent.GOLD_CHEST_TILE_ENTITY.get());
    }

    public static void swapContents(GoldChestTileEntity tileEntityIn, GoldChestTileEntity tileEntityOut) {
    ...
    public int getPlayersUsing(IBlockReader reader, BlockPos pos) {
        ...
        if (blockState.hasTileEntity()) {
            ...
            if (tileEntity instanceof GoldChestTileEntity) {
                return ((GoldChestTileEntity) tileEntity).playersUsing;
            }
        }
        return 0;
    }
    ...
    protected Container createMenu(int id, PlayerInventory player) {
        return new GoldChestContainer(id, player, this);
    }
