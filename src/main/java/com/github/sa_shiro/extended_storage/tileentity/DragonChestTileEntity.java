...
import com.github.sa_shiro.extended_storage.container.DragonChestContainer;
...
public class DragonChestTileEntity extends LockableLootTileEntity {

    private static final int rows = DragonChestContainer.numRows;
    private static final int columns = DragonChestContainer.numColumns;
    protected int playersUsing;
    private String translationKey = "container.dragon_chest";
    private NonNullList<ItemStack> chestContent = NonNullList.withSize(rows * columns, ItemStack.EMPTY);
    private IItemHandlerModifiable items = createHandler();
    private LazyOptional<IItemHandlerModifiable> itemHandler = LazyOptional.of(() -> items);

    public DragonChestTileEntity(TileEntityType<?> typeIn) {
        super(typeIn);
    }

    public DragonChestTileEntity() {
        this(DeferredRegistryEvent.DRAGON_CHEST_TILE_ENTITY.get());
    }

    public static void swapContents(DragonChestTileEntity tileEntityIn, DragonChestTileEntity tileEntityOut) {
    ...
    public int getPlayersUsing(IBlockReader reader, BlockPos pos) {
        ...
        if (blockState.hasTileEntity()) {
            ...
            if (tileEntity instanceof DragonChestTileEntity) {
                return ((DragonChestTileEntity) tileEntity).playersUsing;
            }
        }
        return 0;
    }
    ...
    protected Container createMenu(int id, PlayerInventory player) {
        return new DragonChestContainer(id, player, this);
    }
