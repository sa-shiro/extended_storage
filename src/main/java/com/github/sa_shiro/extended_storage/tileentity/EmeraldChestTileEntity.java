...
import com.github.sa_shiro.extended_storage.container.EmeraldChestContainer;
...
public class EmeraldChestTileEntity extends LockableLootTileEntity {

    private static final int rows = EmeraldChestContainer.numRows;
    private static final int columns = EmeraldChestContainer.numColumns;
    protected int playersUsing;
    private String translationKey = "container.emerald_chest";
    private NonNullList<ItemStack> chestContent = NonNullList.withSize(rows * columns, ItemStack.EMPTY);
    private IItemHandlerModifiable items = createHandler();
    private LazyOptional<IItemHandlerModifiable> itemHandler = LazyOptional.of(() -> items);

    public EmeraldChestTileEntity(TileEntityType<?> typeIn) {
        super(typeIn);
    }

    public EmeraldChestTileEntity() {
        this(DeferredRegistryEvent.EMERALD_CHEST_TILE_ENTITY.get());
    }

    public static void swapContents(EmeraldChestTileEntity tileEntityIn, EmeraldChestTileEntity tileEntityOut) {
    ...
    public int getPlayersUsing(IBlockReader reader, BlockPos pos) {
        ...
        if (blockState.hasTileEntity()) {
            ...
            if (tileEntity instanceof EmeraldChestTileEntity) {
                return ((EmeraldChestTileEntity) tileEntity).playersUsing;
            }
        }
        return 0;
    }
    ...
    protected Container createMenu(int id, PlayerInventory player) {
        return new EmeraldChestContainer(id, player, this);
    }
