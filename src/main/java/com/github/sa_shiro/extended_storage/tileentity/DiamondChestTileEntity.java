...
import com.github.sa_shiro.extended_storage.container.DiamondChestContainer;
...
public class DiamondChestTileEntity extends LockableLootTileEntity {

    private static final int rows = DiamondChestContainer.numRows;
    private static final int columns = DiamondChestContainer.numColumns;
    protected int playersUsing;
    private String translationKey = "container.diamond_chest";
    private NonNullList<ItemStack> chestContent = NonNullList.withSize(rows * columns, ItemStack.EMPTY);
    private IItemHandlerModifiable items = createHandler();
    private LazyOptional<IItemHandlerModifiable> itemHandler = LazyOptional.of(() -> items);

    public DiamondChestTileEntity(TileEntityType<?> typeIn) {
        super(typeIn);
    }

    public DiamondChestTileEntity() {
        this(DeferredRegistryEvent.DIAMOND_CHEST_TILE_ENTITY.get());
    }

    public static void swapContents(DiamondChestTileEntity tileEntityIn, DiamondChestTileEntity tileEntityOut) {
    ...
    public int getPlayersUsing(IBlockReader reader, BlockPos pos) {
        ...
        if (blockState.hasTileEntity()) {
            ...
            if (tileEntity instanceof DiamondChestTileEntity) {
                return ((DiamondChestTileEntity) tileEntity).playersUsing;
            }
        }
        return 0;
    }
    ...
    protected Container createMenu(int id, PlayerInventory player) {
        return new DiamondChestContainer(id, player, this);
    }
