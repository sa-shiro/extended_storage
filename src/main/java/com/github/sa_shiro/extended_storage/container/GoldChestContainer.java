...
public class GoldChestContainer extends Container {

    public static final int numRows = 6;
    public static final int numColumns = 13;
    public final GoldChestTileEntity tileEntity;
    private final IWorldPosCallable canInteractWithCallable;


    public GoldChestContainer(final int windowId, final PlayerInventory playerInventoryIn, final GoldChestTileEntity tileEntity) {
        super(DeferredRegistryEvent.GOLD_CHEST_CONTAINER.get(), windowId);
    ...
    public GoldChestContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
        this(windowId, playerInventory, getTileEntity(playerInventory, data));
    }

    private static GoldChestTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory can't be null!");
        Objects.requireNonNull(data, "data can't be null!");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof GoldChestTileEntity) {
            return (GoldChestTileEntity) tileAtPos;
        }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(canInteractWithCallable, playerIn, DeferredRegistryEvent.GOLD_CHEST.get());
    }
