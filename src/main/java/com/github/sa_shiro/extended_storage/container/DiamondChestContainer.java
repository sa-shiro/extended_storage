...
import com.github.sa_shiro.extended_storage.tileentity.DiamondChestTileEntity;
...
public class DiamondChestContainer extends Container {

    public static final int numRows = 8;
    public static final int numColumns = 15;
    public final DiamondChestTileEntity tileEntity;
    private final IWorldPosCallable canInteractWithCallable;


    public DiamondChestContainer(final int windowId, final PlayerInventory playerInventoryIn, final DiamondChestTileEntity tileEntity) {
        super(DeferredRegistryEvent.DIAMOND_CHEST_CONTAINER.get(), windowId);
    ...
    public DiamondChestContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
        this(windowId, playerInventory, getTileEntity(playerInventory, data));
    }

    private static DiamondChestTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory can't be null!");
        Objects.requireNonNull(data, "data can't be null!");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof DiamondChestTileEntity) {
            return (DiamondChestTileEntity) tileAtPos;
        }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(canInteractWithCallable, playerIn, DeferredRegistryEvent.DIAMOND_CHEST.get());
    }
